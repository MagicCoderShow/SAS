...
public class MD5Helper {
	 ...
	 * @return
	 */
	public static String getTwiceMD5ofString(String str){
	 ...
	 * @return
	 */
	public static String getMD5ofStr(String in) {
		MD5Helper m = new MD5Helper();
		m.md5Init(); // 初始化
		m.md5Update(in.getBytes(), in.length());// 调用MD5的主计算过程
		m.md5Final(); // 输出结果到digest数组中
		for (int i = 0; i < 16; i++) {
			m.resultStr += byteToHEX(m.digest[i]); // 将digest数组中的每个byte型数据转为16进制形式的字符串
		}
		return m.resultStr;
	}
	...
	public static void main(String[] args) throws InstantiationException, IllegalAccessException {
		String s = "123456";
		System.out.println(getMD5ofStr(s));
		System.out.println(getMD5ofStr(s));
	}
